    ...
    uint8_t* present_reading)
{
    if (msg == NULL || completion_code == NULL || sensor_data_size == NULL ||
        sensor_operational_state == NULL || present_reading == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    *completion_code = msg->payload[0];
    ...
    if (response->sensor_data_size > PLDM_SENSOR_OEM_DATA_SIZE_SINT64)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    *sensor_operational_state = response->sensor_operational_state;
    ...
    if (*sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_UINT8 ||
        *sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_SINT8)
    {
        if (payload_length !=
    ...
    else if (*sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_UINT16 ||
             *sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_SINT16)
    {
        if (payload_length !=
    ...
    else if (*sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_UINT32 ||
             *sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_SINT32)
    {
        if (payload_length !=
    ...
    else if (*sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_UINT64 ||
             *sensor_data_size == PLDM_SENSOR_OEM_DATA_SIZE_SINT64)
    {
        if (payload_length !=
