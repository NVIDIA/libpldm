...
{
    if (msg == NULL || transfer_flag == NULL || next_transfer_handle == NULL ||
        completion_code == NULL || file_table_data_start_offset == NULL ||
        file_table_length == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    *file_table_length = payload_length - PLDM_GET_FILE_TABLE_MIN_RESP_BYTES;
...
int decode_read_file_req(const struct pldm_msg* msg, size_t payload_length,
                         ...
                         uint32_t* length)
{
    if (msg == NULL || file_handle == NULL || offset == NULL || length == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                          ...
                          uint32_t* length, size_t* file_data_offset)
{
    ...
    *file_data_offset = sizeof(*file_handle) + sizeof(*offset) +
                        sizeof(*length);
                        ...
                        uint64_t* length)
{
    ...
    struct pldm_new_file_req* request = (struct pldm_new_file_req*)msg->payload;
    *file_type = le16toh(request->file_type);
    *file_handle = le32toh(request->file_handle);
                        ...
                        struct pldm_msg* msg)
{
    ...
    struct pldm_new_file_req* req = (struct pldm_new_file_req*)msg->payload;
    req->file_type = htole16(file_type);
    req->file_handle = htole32(file_handle);
                                ...
                                struct pldm_msg* msg)
{
    ...
    if (command != PLDM_READ_FILE_BY_TYPE && command != PLDM_WRITE_FILE_BY_TYPE)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                        ...
                        uint8_t* file_status)
{
    ...
    struct pldm_file_ack_req* request = (struct pldm_file_ack_req*)msg->payload;
    *file_type = le16toh(request->file_type);
    *file_handle = le32toh(request->file_handle);
                        ...
                        struct pldm_msg* msg)
{
    ...
    struct pldm_file_ack_req* req = (struct pldm_file_ack_req*)msg->payload;
    req->file_type = htole16(file_type);
    req->file_handle = htole32(file_handle);
                                       ...
                                       uint8_t* completion_code)
{
    ...
    if (payload_length != PLDM_NEW_FILE_AVAILABLE_WITH_META_DATA_RESP_BYTES)
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }
    ...
    uint32_t* file_meta_data_4)
{
    ...
    if (payload_length != PLDM_NEW_FILE_AVAILABLE_WITH_META_DATA_REQ_BYTES)
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }
