...
extern int pldm_transport_mctp_demux_get_socket_fd(
...
pldm_requester_rc_t pldm_open(void)
{
    ...
    if (open_transport)
    {
        ...
        /* If someone has externally issued close() on fd then we need to start
         * again. Use `fcntl(..., F_GETFD)` to test whether fd is valid. */
    ...
    /* We retest open_transport as it may have been set to NULL by pldm_close()
     * above. */
    if (!open_transport)
    {
                                  ...
                                  uint8_t** pldm_resp_msg, size_t* resp_msg_len)
{
     ...
     * opened. */
    if (open_transport &&
        mctp_fd == pldm_transport_mctp_demux_get_socket_fd(open_transport))
    {
        using_open_transport = true;
        demux = open_transport;
    }
...
pldm_requester_rc_t pldm_recv(mctp_eid_t eid, int mctp_fd,
                              ...
                              uint8_t** pldm_resp_msg, size_t* resp_msg_len)
{
    pldm_requester_rc_t rc = pldm_recv_any(eid, mctp_fd, pldm_resp_msg,
                                           resp_msg_len);
    struct pldm_msg_hdr* hdr = (struct pldm_msg_hdr*)(*pldm_resp_msg);
    if (rc == PLDM_REQUESTER_SUCCESS && hdr && hdr->instance_id != instance_id)
    {
        free(*pldm_resp_msg);
        *pldm_resp_msg = NULL;
                                   ...
                                   size_t* resp_msg_len)
{
    ...
    PLDM_REQ_FN(eid, mctp_fd, pldm_transport_send_recv_msg, rc, pldm_req_msg,
                              ...
                              const uint8_t* pldm_req_msg, size_t req_msg_len)
{
    ...
    PLDM_REQ_FN(eid, mctp_fd, pldm_transport_send_msg, rc, (void*)pldm_req_msg,
                req_msg_len);
    return rc;
}
