                                                       ...
                                                       size_t* msg_len)
{
    ...
    if (af_mctp->bound && hdr->request)
    {
        ...
        cookie->req.tid = *tid, cookie->req.instance_id = hdr->instance_id,
        cookie->req.type = hdr->type, cookie->req.command = hdr->command;
        cookie->smctp = addr;

        rc = pldm_responder_cookie_track(&af_mctp->cookie_jar, &cookie->req);
        if (rc)
        {
            res = PLDM_REQUESTER_RECV_FAIL;
            goto cleanup_msg;
                                                       ...
                                                       size_t msg_len)
{
    ...
    if (msg_len > INT_MAX || pldm_socket_sndbuf_accomodate(
                                 &(af_mctp->socket_send_buf), (int)msg_len))
    {
        return PLDM_REQUESTER_SEND_FAIL;
    }
...
int pldm_transport_af_mctp_init(struct pldm_transport_af_mctp** ctx)
{
    ...
    if (pldm_socket_sndbuf_init(&af_mctp->socket_send_buf, af_mctp->socket))
    {
                                ...
                                const struct sockaddr_mctp* smctp, size_t len)
{
    ...
    rc = bind(transport->socket, (const struct sockaddr*)smctp, sizeof(*smctp));
    if (rc)
    {
