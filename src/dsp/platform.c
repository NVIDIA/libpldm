    ...
    const size_t possible_states_size, size_t* const actual_size)
{
...

    {
        ...
        for (int i = 0; i < effecter->composite_effecter_count; ++i)
        {
            ...
            states_ptr += (sizeof(*states) - sizeof(states->states) +
    ...
    *actual_size = (sizeof(struct pldm_state_effecter_pdr) +
                    possible_states_size - sizeof(effecter->possible_states));
    ...
    memcpy(effecter->possible_states, possible_states, possible_states_size);
                                         ...
                                         struct pldm_msg* msg)
{
    ...
    if (comp_effecter_count < 0x1 || comp_effecter_count > 0x8 || field == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                        ...
                        uint8_t transfer_crc, struct pldm_msg* msg)
{
    ...
    if (response->completion_code == PLDM_SUCCESS)
    {
        ...
        response->next_data_transfer_handle = htole32(next_data_transfer_hndl);
    ...
    struct pldm_msg* msg)
{
    ...
    if (response->completion_code == PLDM_SUCCESS)
    {
        ...
        if (update_time != NULL)
        {
            memcpy(response->update_time, update_time, PLDM_TIMESTAMP104_SIZE);
        }
        ...
        response->data_transfer_handle_timeout = data_transfer_handle_timeout;
    }
    ...
    uint32_t* largest_record_size, uint8_t* data_transfer_handle_timeout)
{
    ...
    if (msg == NULL || completion_code == NULL || repository_state == NULL ||
        update_time == NULL || oem_update_time == NULL ||
        record_count == NULL || repository_size == NULL ||
        largest_record_size == NULL || data_transfer_handle_timeout == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                       ...
                       struct pldm_msg* msg, size_t payload_length)
{
    ...
    struct pldm_get_pdr_req* request = (struct pldm_get_pdr_req*)msg->payload;
                        ...
                        uint8_t* transfer_crc)
{
    ...
    if (msg == NULL || completion_code == NULL || next_record_hndl == NULL ||
        next_data_transfer_hndl == NULL || transfer_flag == NULL ||
        resp_cnt == NULL || transfer_crc == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                                          ...
                                          uint8_t effecter_value[4])
{
    ...
    rc = pldm_msgbuf_init_cc(buf, PLDM_SET_NUMERIC_EFFECTER_VALUE_MIN_REQ_BYTES,
                             msg->payload, payload_length);
    if (rc)
    {
        return rc;
    ...
    const uint8_t* effecter_value, struct pldm_msg* msg, size_t payload_length)
{
    ...
    {
        if (payload_length != PLDM_SET_NUMERIC_EFFECTER_VALUE_MIN_REQ_BYTES + 1)
        {
            return PLDM_ERROR_INVALID_LENGTH;
        }
    ...
    {
        if (payload_length != PLDM_SET_NUMERIC_EFFECTER_VALUE_MIN_REQ_BYTES + 3)
        {
            return PLDM_ERROR_INVALID_LENGTH;
        }
                                          ...
                                          get_sensor_state_field* field)
{
    ...
    if (msg == NULL || completion_code == NULL || comp_sensor_count == NULL ||
        field == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }

    rc = pldm_msgbuf_init_cc(buf, PLDM_GET_STATE_SENSOR_READINGS_MIN_RESP_BYTES,
                             msg->payload, payload_length);
    if (rc)
    {
        return rc;
                                      ...
                                      size_t* event_data_offset)
{
    ...
    *event_data_offset = sizeof(*format_version) + sizeof(*tid) +
                         sizeof(*event_class);
    ...
    size_t payload_length)
{
    ...
    rc = pldm_msgbuf_init_cc(
        buf, PLDM_POLL_FOR_PLATFORM_EVENT_MESSAGE_MIN_RESP_BYTES, msg->payload,
        payload_length);
    if (rc)
    {
        return rc;
                                       ...
                                       uint8_t* platform_event_status)
{
    ...
    if (msg == NULL || completion_code == NULL || platform_event_status == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    struct pldm_msg* msg)
{
    ...
    request->event_receiver_max_buffer_size = event_receiver_max_buffer_size;
    ...
    uint8_t event_class_count)
{
    ...
    if (msg == NULL || completion_code == NULL || synchrony_config == NULL ||
        synchrony_config_support == NULL ||
        number_event_class_returned == NULL || event_class == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }

    rc = pldm_msgbuf_init_cc(buf, PLDM_EVENT_MESSAGE_SUPPORTED_MIN_RESP_BYTES,
                             msg->payload, payload_length);
    if (rc)
    {
        return rc;
                             ...
                             size_t* event_class_data_offset)
{
    ...
    rc = pldm_msgbuf_init_cc(buf, PLDM_SENSOR_EVENT_DATA_MIN_LENGTH, event_data,
                             event_data_length);
    if (rc)
    {
        return rc;
    ...
    size_t event_class_data_length = event_data_length -
    ...
    *event_class_data_offset = sizeof(*sensor_id) +
                               sizeof(*sensor_event_class_type);
                          ...
                          uint8_t* present_op_state, uint8_t* previous_op_state)
{
    ...
    rc = pldm_msgbuf_init_cc(buf, PLDM_SENSOR_EVENT_SENSOR_OP_STATE_DATA_LENGTH,
    ...
    struct pldm_numeric_sensor_value_pdr* pdr_value)
{
    ...
    rc = pldm_platform_pdr_hdr_validate(&pdr_value->hdr,
    ...
    pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format,
    ...
    uint8_t* pending_value, uint8_t* present_value)
{
    ...
    pldm_msgbuf_extract_effecter_value(buf, *effecter_data_size, pending_value);
    pldm_msgbuf_extract_effecter_value(buf, *effecter_data_size, present_value);
    ...
    size_t* actual_change_records_size, size_t max_change_records_size)
{
    ...
    size_t expected_size = sizeof(event_data_format) +
                           sizeof(number_of_change_records);

    expected_size += sizeof(*event_data_operations) * number_of_change_records;
    expected_size += sizeof(*numbers_of_change_entries) *
    ...
    *actual_change_records_size = expected_size;
    ...
    for (uint8_t i = 0; i < number_of_change_records; ++i)
    {
        ...
        record_data =
            (struct
             pldm_pdr_repository_change_record_data*)(record_data
                                                          ->change_entry +
                                                      record_data
                                              ...
                                              size_t* change_record_data_offset)
{
    ...
    *change_record_data_offset = sizeof(*event_data_format) +
                                 sizeof(*number_of_change_records);
    ...
    size_t* change_entry_data_offset)
{
    ...
    rc = pldm_msgbuf_init_cc(buf, PLDM_PDR_REPOSITORY_CHANGE_RECORD_MIN_LENGTH,
    ...
    uint8_t* previous_state, uint8_t* event_state, uint8_t* present_reading)
{
    ...
    pldm_msgbuf_extract_sensor_value(buf, *sensor_data_size, present_reading);
    ...
    struct pldm_numeric_effecter_value_pdr* pdr_value)
{
    ...
    rc = pldm_platform_pdr_hdr_validate(
        &hdr, PLDM_PDR_NUMERIC_EFFECTER_PDR_MIN_LENGTH, pdr_data_length);
    if (rc)
    {
        return rc;
    }
                                         ...
                                         size_t payload_length)
{
    ...
    rc = pldm_msgbuf_init_errno(buf, PLDM_GET_STATE_EFFECTER_STATES_REQ_BYTES,
                                msg->payload, payload_length);
    if (rc)
    {
        return rc;
    ...
    struct pldm_get_state_effecter_states_resp* resp)
{
    ...
    for (i = 0, field = resp->field; i < comp_effecter_count; i++, field++)
    {
