...
static bool is_comp_resp_code_valid(uint8_t comp_resp_code)
{
    switch (comp_resp_code)
    {
        ...
        default:
            if (comp_resp_code >= PLDM_CRC_VENDOR_COMP_RESP_CODE_RANGE_MIN &&
                comp_resp_code <= PLDM_CRC_VENDOR_COMP_RESP_CODE_RANGE_MAX)
            {
                return true;
            }
            return false;
 ...
 */
static bool is_self_contained_activation_req_valid(
    ...
    struct variable_field* fw_device_pkg_data)
{
    if (data == NULL || fw_device_id_record == NULL ||
        applicable_components == NULL || comp_image_set_version_str == NULL ||
        record_descriptors == NULL || fw_device_pkg_data == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    if ((component_bitmap_bit_length % PLDM_FWUP_COMPONENT_BITMAP_MULTIPLE) !=
        0)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    fw_device_id_record->record_length = le16toh(data_record->record_length);
    ...
    uint16_t applicable_components_length = component_bitmap_bit_length /
    ...
    applicable_components->ptr = data +
                                 sizeof(struct pldm_firmware_device_id_record);
    applicable_components->length = applicable_components_length;

    comp_image_set_version_str->ptr = applicable_components->ptr +
                                      applicable_components->length;
    ...
    {
        fw_device_pkg_data->ptr = record_descriptors->ptr +
                                  record_descriptors->length;
                                        ...
                                        struct variable_field* descriptor_data)
{
    ...
    if (data == NULL || descriptor_type == NULL || descriptor_data == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    struct pldm_descriptor_tlv* entry = (struct pldm_descriptor_tlv*)(data);
    ...
    *descriptor_type = le16toh(entry->descriptor_type);
    ...
    {
        if (descriptor_length != get_descriptor_type_length(*descriptor_type))
        {
            return PLDM_ERROR_INVALID_LENGTH;
        }
    }
    ...
    struct variable_field* descriptor_data)
{
    ...
    *descriptor_title_str_type =
    ...
    descriptor_data->ptr = descriptor_title_str->ptr +
                           descriptor_title_str->length;
    ...
    struct variable_field* comp_version_str)
{
    ...
    comp_version_str->ptr = data +
                            sizeof(struct pldm_component_image_information);
    comp_version_str->length = pldm_comp_image_info->comp_version_string_length;
                                         ...
                                         uint8_t** descriptor_data)
{
    ...
    *completion_code = msg->payload[0];
    ...
    if (payload_length < sizeof(struct pldm_query_device_identifiers_resp))
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }
    ...
    struct variable_field* comp_parameter_table)
{
    ...
    if (!is_string_type_valid(response->active_comp_image_set_ver_str_type) ||
    ...
    if (payload_length > partial_response_length && resp_data->comp_count)
    {
        comp_parameter_table->ptr =
            ...
            resp_data->active_comp_image_set_ver_str_len +
            resp_data->pending_comp_image_set_ver_str_len;
        comp_parameter_table->length = payload_length - partial_response_length;
    }
    else
    {
...
{
    if (data == NULL || component_data == NULL || active_comp_ver_str == NULL ||
        pending_comp_ver_str == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    component_data->comp_classification = le16toh(entry->comp_classification);
    ...
    component_data->active_comp_ver_str_type = entry->active_comp_ver_str_type;
    component_data->active_comp_ver_str_len = entry->active_comp_ver_str_len;
    ...
    component_data->pending_comp_ver_str_len = entry->pending_comp_ver_str_len;
    ...
    size_t payload_length)
{
    ...
    rc = pldm_msgbuf_init_cc(buf, PLDM_QUERY_DOWNSTREAM_IDENTIFIERS_REQ_BYTES,
                             msg->payload, payload_length);
    if (rc)
    {
        return rc;
    ...
    struct variable_field* downstream_devices)
{
    ...
    pldm_msgbuf_extract(buf, resp_data->number_of_downstream_devices);
    rc = pldm_msgbuf_span_required(buf, resp_data->downstream_devices_length,
                              ...
                              struct pldm_msg* msg, size_t payload_length)
{
    ...
    if (payload_length !=
        sizeof(struct pldm_request_update_req) + comp_img_set_ver_str->length)
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }
...
{
    if (msg == NULL || completion_code == NULL || fd_meta_data_len == NULL ||
        fd_will_send_pkg_data == NULL || !payload_length)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    struct pldm_msg* msg, size_t payload_length)
{
    ...
    if (payload_length !=
        sizeof(struct pldm_pass_component_table_req) + comp_ver_str->length)
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }

    if ((comp_ver_str_len == 0) || (comp_ver_str_len != comp_ver_str->length))
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                                 ...
                                 uint16_t* time_before_req_fw_data)
{
    ...
    *completion_code = msg->payload[0];
    ...
    if (!is_comp_compatibility_resp_valid(response->comp_compatibility_resp))
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                                 ...
                                 struct pldm_msg* msg, size_t payload_length)
{
    ...
    if (!is_self_contained_activation_req_valid(self_contained_activation_req))
    {
        return PLDM_ERROR_INVALID_DATA;
    }
                                  ...
                                  uint16_t* estimated_time_activation)
{
    ...
    *estimated_time_activation = le16toh(response->estimated_time_activation);
                           ...
                           bitfield32_t* update_option_flags_enabled)
{
    ...
    *completion_code = msg->payload[0];
    ...
    {
        if (response->aux_state != PLDM_FD_IDLE_LEARN_COMPONENTS_READ_XFER)
        {
            return PLDM_ERROR_INVALID_DATA;
        }
    }
