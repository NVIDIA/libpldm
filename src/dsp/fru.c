    ...
    uint32_t* checksum)
{
    if (msg == NULL || completion_code == NULL ||
        ...
        total_record_set_identifiers == NULL || total_table_records == NULL ||
        checksum == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    uint32_t checksum, struct pldm_msg* msg)
{
    ...
    if (response->completion_code == PLDM_SUCCESS)
    {
        ...
        response->fru_table_maximum_size = htole32(fru_table_maximum_size);
...
{
    return p >= (const struct pldm_fru_record_data_format*)((uint8_t*)table +
                                                            table_size);
}
                             ...
                             uint16_t rsi, uint8_t rt, uint8_t ft)
{
    ...
    {
        if ((record_data_src->record_set_id != htole16(rsi) && rsi != 0) ||
            (record_data_src->record_type != rt && rt != 0))
        {
            ...
            for (int i = 0; i < record_data_src->num_fru_fields; i++)
            {
                ...
                tlv = (const struct pldm_fru_record_tlv*)((char*)tlv + len);
            }
            record_data_src = (const struct pldm_fru_record_data_format*)(tlv);
            continue;
        }
    ...
    struct pldm_msg* msg, size_t payload_length)
{
    ...
    if (payload_length != sizeof(struct pldm_get_fru_record_by_option_req))
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }
    ...
    uint8_t* transfer_op_flag)
{
    if (msg == NULL || data_transfer_handle == NULL ||
        ...
        record_type == NULL || field_type == NULL || transfer_op_flag == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    struct variable_field* fru_structure_data)
{
    ...
    *transfer_flag = resp->transfer_flag;
    ...
    fru_structure_data->length = payload_length -
    ...
    size_t max_fru_record_table_length)
{
    ...
    *fru_record_table_length = payload_length -
...
{
    if (msg == NULL || data_transfer_handle == NULL || transfer_flag == NULL ||
        fru_table_data == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    *transfer_flag = req->transfer_flag;
    fru_table_data->length = payload_length -
