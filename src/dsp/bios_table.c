    ...
    const struct pldm_bios_string_table_entry* entry, char* buffer, size_t size)
{
    ...
    size_t length = pldm_bios_table_string_entry_decode_string_length(entry);
                                                     ...
                                                     uint8_t def_num)
{
    return sizeof(struct pldm_bios_attr_table_entry) -
           MEMBER_SIZE(pldm_bios_attr_table_entry, metadata) + sizeof(pv_num) +
    ...
    const struct pldm_bios_table_attr_entry_enum_info* info)
{
    ...
    attr_entry->metadata[1 + info->pv_num * sizeof(uint16_t)] = info->def_num;
    ...
    const struct pldm_bios_attr_table_entry* entry)
{
    ...
    return entry
        ->metadata[sizeof(uint8_t) /* pv_num */ + sizeof(uint16_t) * pv_num];
}
...
static ssize_t attr_table_entry_length_enum(const void* arg)
{
    ...
    size_t len = pldm_bios_table_attr_entry_enum_encode_length(pv_num, def_num);
    if (len > SSIZE_MAX)
    {
        return -1;
    }
...
static ssize_t attr_table_entry_length_string(const void* entry)
{
    uint16_t def_str_len =
        pldm_bios_table_attr_entry_string_decode_def_string_length(entry);
    size_t len = pldm_bios_table_attr_entry_string_encode_length(def_str_len);
    if (len > SSIZE_MAX)
    {
        return -1;
    }
    ...
    const char** errmsg)
{
    ...
    if (info->lower_bound > info->upper_bound)
    {
        set_errmsg(errmsg, "LowerBound should not be greater than UpperBound");
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    if ((info->default_value - info->lower_bound) % info->scalar_increment != 0)
    {
...
static ssize_t attr_table_entry_length(const void* table_entry)
{
    ...
    const struct table_entry_length* attr_table_entry =
        find_table_entry_length_by_type(entry->attr_type, attr_table_entries,
...
{
    return sizeof(struct pldm_bios_attr_val_table_entry) - 1 + sizeof(count) +
           count;
}
    ...
    uint8_t count, const uint8_t* handles)
{
    ...
    size_t length = pldm_bios_table_attr_value_entry_encode_enum_length(count);
    BUFFER_SIZE_EXPECT(entry_length, length);
...
{
    uint8_t number = pldm_bios_table_attr_value_entry_enum_decode_number(entry);
    size_t len = pldm_bios_table_attr_value_entry_encode_enum_length(number);
    if (len > SSIZE_MAX)
    {
        return -1;
    }
    ...
    struct variable_field* current_string)
{
    ...
    current_string->ptr = entry->value +
                          sizeof(uint16_t); // sizeof(CurrentStringLength)
}
...
{
    return sizeof(struct pldm_bios_attr_val_table_entry) - 1 + sizeof(uint64_t);
}
...
{
    POINTER_CHECK(entry);
    size_t length = pldm_bios_table_attr_value_entry_encode_integer_length();
                                          ...
                                          equal_handler equal, const void* key)
{
    ...
    const void* entry = pldm_bios_table_entry_find_by_iter(iter, key, equal);
                                          ...
                                          const char* str)
{
    ...
    return pldm_bios_table_entry_find_from_table(
        table, length, PLDM_BIOS_STRING_TABLE, string_table_string_equal, &arg);
}

static bool attr_table_handle_equal(const void* entry, const void* key)
{
    uint16_t handle = *(uint16_t*)key;
    return pldm_bios_table_attr_entry_decode_attribute_handle(entry) == handle;
}
...
{
    return pldm_bios_table_entry_find_from_table(
        table, length, PLDM_BIOS_ATTR_TABLE, attr_table_handle_equal, &handle);
}
...
{
    return pldm_bios_table_entry_find_from_table(
        table, length, PLDM_BIOS_ATTR_VAL_TABLE, attr_value_table_handle_equal,
        &handle);
}
    ...
    size_t* dest_length, const void* entry, size_t entry_length)
{
    ...
    size_t pad_checksum_size = pldm_bios_table_pad_checksum_size(copied_length);
    ...
    rc = pldm_bios_table_append_pad_checksum_check(dest_table, buffer_length,
                                                   &copied_length);
    if (rc == PLDM_SUCCESS)
    {
