                                            ...
                                            uint32_t* next_record_handle)
{
    ...
    while (record != NULL)
    {
        if (record->record_handle == record_handle)
        {
            *size = record->size;
            *data = record->data;
            *next_record_handle = get_next_record_handle(repo, record);
            return record;
        }
        record = record->next;
    }
    ...
    uint16_t* container_id)
{
    ...
    {
        struct pldm_pdr_fru_record_set* fru =
            (struct pldm_pdr_fru_record_set*)(data +
                                              sizeof(struct pldm_pdr_hdr));
                            ...
                            uint8_t tid, uint8_t tl_eid, bool valid_bit)
{
    ...
    uint32_t size = 0;
    const pldm_pdr_record* record;
    record = pldm_pdr_find_record_by_type(repo, PLDM_TERMINUS_LOCATOR_PDR, NULL,
                                          &out_data, &size);
    ...
    do
    {
        if (record != NULL)
        {
            ...
            if (pdr->terminus_handle == terminus_handle && pdr->tid == tid &&
                value->eid == tl_eid)
            {
    ...
    uint32_t range_exclude_end_handle, uint16_t* container_id)
{
    ...
    for (record = repo->first; record; record = record->next)
    {
        ...
        // The expectation here is the pldm_pdr_hdr is the first field of the
        // record data
                                           ...
                                           uint16_t entity_type)
{
    ...
    while (start->next_sibling != NULL)
    {
        ...
        if (this_type == entity_type && (this_type != next->entity.entity_type))
        {
            break;
        }
    ...
    uint8_t association_type)
{
    return pldm_entity_association_tree_add_entity(
        tree, entity, entity_instance_number, parent, association_type, false,
        true, 0xffff);
}
    ...
    uint16_t container_id)
{
    ...
    else if (parent != NULL && parent->first_child == NULL)
    {
        ...
        {
            node->entity.entity_container_id = entity->entity_container_id;
        }
    ...
    else
    {
        pldm_entity_node* start = parent == NULL ? tree->root
                                                 : parent->first_child;
        pldm_entity_node* prev = find_insertion_at(start, entity->entity_type);
        if (!prev)
        {
            free(node);
            return NULL;
        ...
        node->entity.entity_container_id = prev->entity.entity_container_id;
        node->remote_container_id = entity->entity_container_id;
    }
...
bool pldm_is_current_parent_child(pldm_entity_node* parent, pldm_entity* node)
{
    ...
    while (curr != NULL)
    {
        if (node->entity_type == curr->entity.entity_type &&
            node->entity_instance_num == curr->entity.entity_instance_num)
        {
            return true;
        }
        curr = curr->next_sibling;
                                            ...
                                            uint32_t record_handle)
{
    ...
    if (num_logical_children)
    {
        uint16_t logical_pdr_size =
            sizeof(struct pldm_pdr_hdr) + sizeof(uint16_t) + sizeof(uint8_t) +
                                      ...
                                      uint32_t record_handle)
{
    ...
    {
        rc = entity_association_pdr_add_entry(curr, repo, is_remote,
    ...
    uint32_t record_handle)
{
    ...
    entity_association_pdr_add(node, repo, entities, num_entities, is_remote,
                               terminus_handle, record_handle);
                                                   ...
                                                   bool is_remote)
{
    ...
    if (!is_remote || node->remote_container_id == entity->entity_container_id)
    {
