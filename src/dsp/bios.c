...
{
    if (msg == NULL || seconds == NULL || minutes == NULL || hours == NULL ||
        day == NULL || month == NULL || year == NULL || completion_code == NULL)
    {
        return PLDM_ERROR_INVALID_DATA;
    }
    ...
    struct variable_field* attribute_data)
{
    ...
    *completion_code = response->completion_code;
    ...
    if (payload_length <= PLDM_GET_BIOS_ATTR_CURR_VAL_BY_HANDLE_MIN_RESP_BYTES)
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }
...
{
    if (msg == NULL || transfer_handle == NULL || transfer_op_flag == NULL ||
    ...
    size_t attribute_length, struct pldm_msg* msg)
{
    ...
    if (response->completion_code == PLDM_SUCCESS)
    {
        ...
        {
            memcpy(response->attribute_data, attribute_data, attribute_length);
        }
    }
    return PLDM_SUCCESS;
}
    ...
    struct variable_field* attribute)
{
    ...
    *transfer_flag = request->transfer_flag;
    attribute->length = payload_length -
                              ...
                              struct pldm_msg* msg, size_t payload_length)
{
    ...
    if (PLDM_SET_BIOS_TABLE_MIN_REQ_BYTES + table_length != payload_length)
    {
        return PLDM_ERROR_INVALID_LENGTH;
    }
