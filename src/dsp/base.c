                             ...
                             uint8_t* completion_code, bitfield8_t* commands)
{
    ...
    *completion_code = msg->payload[0];
    ...
    memcpy(&(commands->byte), response->commands, PLDM_MAX_CMDS_PER_TYPE / 8);
...
{
    if (msg == NULL || next_transfer_handle == NULL || transfer_flag == NULL ||
...
int encode_set_tid_req(uint8_t instance_id, uint8_t tid, struct pldm_msg* msg)
{
    ...
    struct pldm_set_tid_req* request = (struct pldm_set_tid_req*)msg->payload;
    request->tid = tid;
    ...
    uint32_t* section_offset, uint32_t* section_length)
{
    ...
    if (sec_offset == 0 && (request->transfer_opflag != PLDM_XFER_FIRST_PART &&
